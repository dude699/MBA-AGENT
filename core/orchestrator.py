...
class Orchestrator:
    ...
    async def force_apply(
        ...
        *,
        ...
        """
        User-initiated single-job apply (one-tap from Telegram).

        Used for the MID-score (40-79) band where the system saved the job
        but did NOT auto-apply.  When the user taps [Apply] on the digest
        card, the dashboard calls this method.

        Behaviour:
          • Loads the job + (per-user) score from DB.
          • Honours the Risk Governor for the portal unless bypass_risk=True
            (admin override).  A paused portal still hard-blocks.
          • Bypasses the apply-window by default — user-initiated taps are
            assumed time-sensitive.
          • Marks `source='tap'` and stamps user_id on the queue row so the
            audit log + applied_jobs row show it was user-driven.

        Returns:
            {"job_id", "outcome", "user_id", "score", "applied_at", "error"}
        """
        log.info("orch.force_apply job_id=%s user_id=%s source=%s",
        ...
        try:
            ...
            return {"job_id": job_id, "outcome": "FETCH_FAILED",
                    "user_id": user_id, "error": str(e)}
        if job is None:
            return {"job_id": job_id, "outcome": "NOT_FOUND",
                    "user_id": user_id, "error": "job not in DB"}

        # 2. Resolve score (per-user if available, else global)
        score = 0
        get_user_score = getattr(self.db, "get_user_score", None)
        if user_id is not None and get_user_score is not None:
            try:
                bd = await get_user_score(user_id, job_id)
                if bd is not None:
                    score = int(getattr(bd, "final_score", 0))
            except Exception:
                pass
        if score == 0:
            try:
                gs = getattr(self.db, "fetch_score", None)
                if gs:
                    bd = await gs(job_id)
                    if bd is not None:
                        score = int(getattr(bd, "final_score", 0))
        ...
        if not bypass_risk:
            try:
                mult, paused = await self.risk.apply(job.portal)
            except Exception:
                mult, paused = 1.0, False
            if paused:
                await self.on_event("force_apply_blocked", {
                    "job_id": job_id, "user_id": user_id,
                    "portal": job.portal, "reason": "portal_paused",
                })
                return {"job_id": job_id, "outcome": "PORTAL_PAUSED",
                        "user_id": user_id, "score": score,
                        "error": "Risk Governor has paused this portal."}

        # 4. Apply window — bypassed by default for user taps
        if not bypass_window and not is_apply_window_open(job.portal, _now_hhmm()):
            return {"job_id": job_id, "outcome": "OUTSIDE_WINDOW",
                    "user_id": user_id, "score": score,
                    "error": "Outside this portal's preferred apply window."}

        # 5. Build the queue row + dispatch directly (don't wait for tick)
        now = datetime.now(timezone.utc)
        urgency = self._deadline_urgency(job)
        row = QueueRow(
            job_id            = job_id,
            portal            = job.portal,
            score             = score or 0,
            deadline_urgency  = urgency,
            apply_window_open = True,                 # forced
            risk_level        = self._risk_level(score or 0, urgency),
            state             = QueueState.QUEUED,
            attempts          = 0,
            queued_at         = now,
            rescore_at        = now + timedelta(hours=2),
        )
        # Stamp multi-tenant attrs the DAO will persist
        row.user_id        = user_id                  # type: ignore[attr-defined]
        row.source         = source                   # type: ignore[attr-defined]
        try:
            await self.db.upsert_queue_row(row)
        except Exception:
            log.exception("orch.force_apply.upsert_failed job_id=%s", job_id)

        await self.on_event("force_apply_dispatch", {
            "job_id": job_id, "user_id": user_id,
            "portal": job.portal, "score": score, "source": source,
        })

        # 6. Dispatch immediately
        try:
            outcome = await self._dispatch_one(row)
        except Exception as e:                                        # noqa: BLE001
            log.exception("orch.force_apply.dispatch_failed job_id=%s", job_id)
            return {"job_id": job_id, "outcome": "DISPATCH_ERROR",
                    "user_id": user_id, "score": score, "error": str(e)}
    ...
    async def force_skip(
        self, job_id: str, user_id: int | None = None,
    ) -> dict[str, Any]:
        """User dismissed a MID-score job — record as SKIPPED for the learner."""
        marker = getattr(self.db, "mark_user_action", None)
        if marker is not None:
            try:
                await marker(user_id, job_id, "SKIPPED")
            except Exception:
                log.exception("orch.force_skip.mark_failed job_id=%s", job_id)
        await self.on_event("force_skip", {"job_id": job_id, "user_id": user_id})
        return {"job_id": job_id, "outcome": "SKIPPED", "user_id": user_id}
