...
# Multi-tenant + per-user scoring (NEXUS v0.2 — added in PR #64)
from core.access_control import AccessControl, InMemoryAccessDB
from core.cv_intake import CVIntake
from core.scoring_loop import ScoringLoop
...
class NexusRuntime:
    ...
    def __init__(
        ...
        # Multi-tenant + per-user scoring (NEXUS v0.2)
        self._access:    Optional[AccessControl] = None
        self._cv_intake: Optional[CVIntake]      = None
        self._scoring_loop: Optional[ScoringLoop] = None
    ...
    async def start(self) -> NexusBootReport:
        ...
        # ── Auto-bind real Postgres DAO when DATABASE_URL is set ────────
        # This must run BEFORE every layer that takes a `db=` so all of
        # them get the real DAO (multi-tenant rows, per-user job_scores,
        # rate-limit buckets, audit log, etc.).
        await self._maybe_bind_real_db()
        ...
        # Multi-tenant — AccessControl (RBAC + HMAC + audit + rate limit)
        try:
            access_db = self._db if hasattr(self._db, "audit_log") else InMemoryAccessDB()
            self._access = AccessControl(
                db          = access_db,
                hmac_secret = os.getenv("NEXUS_CALLBACK_SECRET") or None,
            )
            self.report.layers_ok.append("L0a_access")
        except Exception as e:                                          # noqa: BLE001
            self.report.layers_fail["L0a_access"] = str(e)
            log.exception("Access control init failed")

        # Multi-tenant — CV intake pipeline (parse + encrypt + embed + persist)
        try:
            self._cv_intake = CVIntake(
                db             = self._db,
                access         = self._access,
                on_cv_ready    = self._on_cv_ready_trigger_rescore,
            )
            self.report.layers_ok.append("L0b_cv_intake")
        except Exception as e:                                          # noqa: BLE001
            self.report.layers_fail["L0b_cv_intake"] = str(e)
            log.exception("CV intake init failed")

        # Layer 9 — Telegram Dashboard (multi-tenant + CV upload + RBAC inject)
        try:
            self._dash = TelegramDashboard(
                ...
                cfg       = DashboardConfig(),
                access    = self._access,
                cv_intake = self._cv_intake,
            )
            # Don't start its own polling loop here — main.py wires it via
            # core.telegram_bridge onto PRISM's existing TGApplication.
            # However, when we're booted standalone (no PRISM bot), the
            # operator can still set NEXUS_STANDALONE_BOT=true to run a
            # dedicated poller for testing.
            if os.getenv("NEXUS_STANDALONE_BOT", "").lower() in ("1", "true", "yes"):
                await self._dash.start()
                self.report.dashboard_ok = bool(self._dash._app)        # noqa: SLF001
            else:
                self.report.dashboard_ok = False  # mounted via bridge later
        ...
        # Layer 3b — Scoring Loop (off-hours per-user CV-match + Bayesian learner)
        try:
            ...
            self.report.layers_ok.append("L3b_scoring_loop")
        except Exception as e:                                          # noqa: BLE001
            self.report.layers_fail["L3b_scoring_loop"] = str(e)
            log.exception("Scoring loop init failed")
    ...
    # ─────────────────────── auto-bind Postgres DAO ────────────────────
    async def _maybe_bind_real_db(self) -> None:
        """
        When DATABASE_URL is set, instantiate core.nexus_db.NexusDB and bind
        it as the DAO for orchestrator / risk / access / cv_intake / scoring
        loop. Falls back silently to the _NullOrchestratorDB stub if
        anything goes wrong (asyncpg missing, schema not applied, etc.).
        """
        url = os.getenv("DATABASE_URL", "").strip()
        if not url:
            log.info("nexus.db.skip — DATABASE_URL not set, using in-memory stubs")
            return
        try:
            from core.nexus_db import NexusDB                            # type: ignore
        except Exception as e:                                          # noqa: BLE001
            log.warning("nexus.db.import_failed %s — using stubs", e)
            return
        try:
            real_db = NexusDB(database_url=url)
            await real_db.connect()
            self._db = real_db
            self.report.db_backend = type(real_db).__name__
            log.info("nexus.db.bound backend=%s", self.report.db_backend)
        except Exception as e:                                          # noqa: BLE001
            log.warning("nexus.db.connect_failed %s — using stubs", e)

    async def _on_cv_ready_trigger_rescore(self, user_id: int) -> None:
        """Hook called by CVIntake after a successful upload: kick the loop."""
        if self._scoring_loop is not None:
            try:
                await self._scoring_loop.enqueue_user(user_id)
            except Exception:                                            # noqa: BLE001
                log.exception("nexus.cv_ready.enqueue_failed user_id=%s", user_id)

    async def stop(self) -> None:
        log.info("nexus.runtime.stop")
        self._stopping.set()
        for t in (self._tick_task, self._rescore_task, self._scoring_task):
            if t and not t.done():
                t.cancel()
                try:
                    await t
        ...
        if self._scoring_loop is not None:
            try:
                ...
                log.exception("scoring loop stop failed")
        if self._dash:
            try:
    ...
    def bind_db(self, real_db: Any) -> None:
        ...
        Propagates the new DAO to every layer that holds a reference:
        risk governor, orchestrator, access control, cv intake, scoring
        loop, and dedup engine.
        """
        self._db = real_db
        self.report.db_backend = type(real_db).__name__
        if self._risk:          self._risk.db          = real_db
        if self._orch:          self._orch.db          = real_db
        if self._access:        self._access.db        = real_db
        if self._cv_intake:     self._cv_intake.db     = real_db
        if self._scoring_loop:  self._scoring_loop.db  = real_db
        if self._dedup:
            try:    self._dedup.db = real_db
