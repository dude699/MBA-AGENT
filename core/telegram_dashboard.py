...
class DashboardConfig:
    ...
    chat_id: str = TELEGRAM_CHAT_ID or ""           # legacy single-chat mode
    enable_digest: bool = True
    digest_hour_ist: int = 21  # 9 PM IST
    multi_tenant: bool = True                        # NEW: gate via access_control
    super_admin_tg_id: int = 1284690336              # MD Abuzar Salim
...
class TelegramDashboard:
    ...
    Multi-tenant mode (default):
      - Auth is by Telegram user_id, not chat_id.
      - Access is granted via access_control.AccessControl (RBAC + audit).
      - MID-score [Apply] callbacks pass through orch.force_apply(job_id, user_id).
      - Unknown users get an "Access pending" message; admin gets approve/reject.
    """
    ...
    def __init__(
        ...
        access: Optional[Any] = None,                # NEW: AccessControl instance
        cv_intake: Optional[Any] = None,             # NEW: CVIntake instance
    ) -> None:
        self.orch = orch
        self.vault = vault
        self.intel = intel
    ...
    async def start(self) -> None:
        ...
        self._app.add_handler(CommandHandler("me",     self._cmd_me))
        self._app.add_handler(CommandHandler("cv",     self._cmd_cv))
        self._app.add_handler(CommandHandler("auto",   self._cmd_auto))
        self._app.add_handler(CommandHandler("access", self._cmd_access))

        # CV upload via document message (PDF/DOCX/TXT)
    ...
    def _is_authorised(self, update: Update) -> bool:
        """Legacy chat-id gate (kept for the single-tenant `cfg.chat_id` path)."""
    ...
    async def _resolve_user(self, update: Update) -> Optional[Any]:
        """
        Multi-tenant gate. Returns the user record (with role + auto_apply_on)
        if the Telegram user has an active grant; otherwise sends an "Access
        pending" message, notifies the super-admin with approve/reject buttons,
        and returns None.

        Falls back to the legacy chat-id check when access_control is absent
        or multi_tenant=False.
        """
        if not self.cfg.multi_tenant or self.access is None:
            return self._is_authorised(update)         # legacy bool
        ...
        try:
            user = await self.access.resolve_telegram_user(
                tg_id=tg_id,
                username=getattr(tg_user, "username", None),
                first_name=getattr(tg_user, "first_name", None),
            )
        except Exception:
            logger.exception("access.resolve_telegram_user failed")
            user = None

        if user is None:
            # Unknown — record request + ping super-admin
            await update.message.reply_text(
                "🔒 *Access pending.*\n"
                "I've notified the admin. You'll get a message here once approved.",
                parse_mode=ParseMode.MARKDOWN,
            )
            try:
                await self.access.record_access_request(
                    tg_id=tg_id,
                    username=getattr(tg_user, "username", None),
                    first_name=getattr(tg_user, "first_name", None),
                )
                await self._notify_admin_access_request(tg_user)
            except Exception:
                logger.exception("notify admin access request failed")
            return None
    ...
    async def _notify_admin_access_request(self, tg_user) -> None:
        if not (self._app and self.cfg.super_admin_tg_id):
            return
        try:
            kb = InlineKeyboardMarkup([[
                InlineKeyboardButton("✅ Approve STD", callback_data=f"grant:standard:{tg_user.id}"),
                InlineKeyboardButton("⭐ POWER",       callback_data=f"grant:power:{tg_user.id}"),
                InlineKeyboardButton("🛑 Reject",      callback_data=f"grant:deny:{tg_user.id}"),
            ]])
            await self._app.bot.send_message(
                chat_id=self.cfg.super_admin_tg_id,
                text=(
                    f"*🆕 Access request*\n"
                    f"User: `{tg_user.id}` "
                    f"(@{getattr(tg_user, 'username', '') or '—'})\n"
                    f"Name: _{getattr(tg_user, 'first_name', '') or '—'}_"
                ),
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=kb,
            )
    ...
    async def _cmd_help(self, update: Update, context) -> None:
        user = await self._resolve_user(update) if self.cfg.multi_tenant else self._is_authorised(update)
        if user in (None, False):
            return
        role = str(getattr(user, "role", "")).upper() if user not in (True, None) else ""
        is_admin = role == "ADMIN"
        is_power = role in ("POWER", "POWER_USER", "ADMIN")
        common = (
            "*👤 Your commands:*\n"
            "/me — your role + auto-apply state + CV status\n"
            "/cv — upload your CV (PDF/DOCX/TXT)\n"
            "/pending — your manual-review queue (40-79 scores)\n"
            "/score <url> — score one job for you\n"
            "/help — this message\n"
        )
        power = (
            "\n*⭐ Power-user:*\n"
            "/auto on|off — toggle auto-apply (≥ 80 score)\n"
        ...
        admin = (
            "\n*🛡 Admin:*\n"
            "/access — pending requests + active grants\n"
            "/pause <portal> · /resume <portal>\n"
            "/vault — session vault health\n"
            "/health · /risk — Risk Governor signals\n"
            "/captcha <answer> — supply pending CAPTCHA answer\n"
        ) if is_admin else ""
        await update.message.reply_text(
            common + power + admin, parse_mode=ParseMode.MARKDOWN,
        )
    ...
    async def _cmd_me(self, update: Update, context) -> None:
        """Show the caller's role + auto-apply state + CV status."""
        user = await self._resolve_user(update)
        if user in (None, False):
            return
        # Try to fetch CV summary
        cv_text = ""
        try:
            if self.cv_intake is not None:
                summary = await self.cv_intake.get_user_cv_summary(user.user_id)
                if summary:
                    cv_text = (
                        f"\n*CV:* `{summary.get('filename', '?')}` "
                        f"({summary.get('chars', 0)} chars, "
                        f"uploaded {summary.get('uploaded_at_human', '?')})"
                    )
        except Exception:
            logger.debug("cv summary fetch failed", exc_info=True)
        await update.message.reply_text(
            f"*👤 Your NEXUS profile*\n"
            f"User ID: `{user.user_id}`\n"
            f"Role: *{getattr(user, 'role', 'STANDARD')}*\n"
            f"Auto-apply: *{'ON' if getattr(user, 'auto_apply_on', False) else 'OFF'}*"
            + cv_text,
            parse_mode=ParseMode.MARKDOWN,
        )

    async def _cmd_cv(self, update: Update, context) -> None:
        """Prompt the user to upload their CV (PDF/DOCX/TXT)."""
        user = await self._resolve_user(update)
        if user in (None, False):
            return
        await update.message.reply_text(
            "*📄 Send me your CV*\n"
            "Reply to this message with a PDF, DOCX, or TXT file.\n"
            "_Max 5 MB. The text is parsed, embedded, and used for per-job "
            "match scoring during off-hours (02:00-06:00 IST)._",
            parse_mode=ParseMode.MARKDOWN,
        )

    async def _cmd_auto(self, update: Update, context) -> None:
        """Toggle auto-apply for ADMIN/POWER users (STANDARD always one-tap)."""
        user = await self._resolve_user(update)
        if user in (None, False):
            return
        role = str(getattr(user, "role", "STANDARD")).upper()
        if role not in ("ADMIN", "POWER", "POWER_USER"):
            await update.message.reply_text(
                "🔒 Auto-apply requires *POWER* or *ADMIN* role.\n"
                "STANDARD users always confirm via one-tap [Apply].",
                parse_mode=ParseMode.MARKDOWN,
            )
            return
        args = context.args or []
        ...
        if args and args[0].lower() in ("on", "true", "yes", "1"):
            ...
            new_state = not bool(getattr(user, "auto_apply_on", False))
        try:
            await self.access.set_auto_apply(user.user_id, new_state)
        except Exception:
            logger.exception("set_auto_apply failed")
            await update.message.reply_text("⚠️ Could not update — see logs.")
            return
        await update.message.reply_text(
            f"✅ Auto-apply is now *{'ON' if new_state else 'OFF'}*.",
            parse_mode=ParseMode.MARKDOWN,
        )
        await self._audit("cmd_auto", {"user_id": user.user_id, "auto": new_state})

    async def _cmd_access(self, update: Update, context) -> None:
        """Admin-only: list pending requests + active grants."""
        user = await self._resolve_user(update)
        if user in (None, False):
            return
        if str(getattr(user, "role", "")).upper() != "ADMIN":
            ...
            return
        try:
            pending = await self.access.list_pending_requests()
            grants  = await self.access.list_active_grants()
        except Exception:
            logger.exception("/access fetch failed")
            await update.message.reply_text("⚠️ Could not fetch — see logs.")
            return
        ...
        if pending:
            ...
            for p in pending[:10]:
                lines.append(
                    f"• `{p.get('tg_id')}` "
                    f"@{p.get('username') or '—'} "
                    f"_{p.get('first_name') or ''}_"
                )
        else:
            lines.append("_No pending requests._")
        lines.append("")
        if grants:
            lines.append(f"*Active grants ({len(grants)}):*")
            for g in grants[:15]:
                lines.append(
                    f"• `{g.get('tg_id')}` — *{g.get('role')}* "
                    f"(auto={g.get('auto_apply_on')})"
                )
        await update.message.reply_text(
            "\n".join(lines), parse_mode=ParseMode.MARKDOWN,
        )
    ...
    # ────────────────────────────────────────────────────────────────────
    #  Document upload (CV intake)
    # ────────────────────────────────────────────────────────────────────

    async def _on_document(self, update: Update, context) -> None:
        """Receive a CV upload, hand off to CVIntake pipeline."""
        user = await self._resolve_user(update)
        if user in (None, False):
            return
        if self.cv_intake is None:
            await update.message.reply_text(
                "⚠️ CV intake not initialised on this deployment.",
            )
            return
        ...
        if (doc.file_size or 0) > 5 * 1024 * 1024:
            await update.message.reply_text("❌ File too large (>5 MB).")
            return

        await update.message.reply_text("📥 Got it — parsing & embedding…")
        try:
        ...
        try:
            result = await self.cv_intake.ingest_telegram_upload(
            ...
            logger.exception("cv_intake.ingest failed")
            await update.message.reply_text("❌ CV parsing failed — see logs.")
            return

        if not result or not result.get("ok"):
            await update.message.reply_text(
                f"❌ {result.get('error', 'CV parse failed')}",
            )
            return

        await update.message.reply_text(
            f"✅ *CV stored.*\n"
            f"Length: {result.get('chars', 0)} chars · "
            f"Embedding: {result.get('embed_dim', 0)}-dim\n"
            f"_Backfilling job-match scores in the background._",
            parse_mode=ParseMode.MARKDOWN,
        )
        await self._audit("cv_upload", {
            "user_id": user.user_id,
    ...
    async def _on_callback(self, update: Update, context) -> None:
        # Resolve the calling Telegram user (multi-tenant) so per-user actions
        # like Apply/Skip carry the user_id forward to the orchestrator.
        user = None
        if self.cfg.multi_tenant and self.access is not None:
            try:
                tg_id = int(update.effective_user.id)
            except Exception:
                tg_id = 0
            # Special case: super-admin grant callbacks always go through
            data_peek = (update.callback_query.data or "")
            if data_peek.startswith("grant:") and tg_id == int(self.cfg.super_admin_tg_id):
                user = await self.access.resolve_telegram_user(
                    tg_id=tg_id,
        ...
        try:
            ...
            if verb == "apply":
                result = await self.orch.force_apply(payload, user_id=user_id, source="tap")
                ok = result.get("outcome") in ("SUCCESS", "RETRY")
                ...
                res = await self.orch.force_skip(payload, user_id=user_id)
                await q.edit_message_text(
                    (q.message.text or "")
                    + f"\n\n🛑 Skipped" + (f" — {res.get('error')}" if res.get('error') else ""),
                )
            elif verb == "snooze":
                # Re-queue this job for re-display in the next digest (no apply)
                await q.edit_message_text(
                    (q.message.text or "") + "\n\n⏰ Snoozed — will resurface in next digest.",
                )
            elif verb == "grant":
                # admin-only: grant:role:tg_id  (role ∈ standard|power|admin|deny)
                if not user or getattr(user, "role", None) not in ("admin", "ADMIN"):
                    await q.message.reply_text("🛑 Admin only.")
                    return
                role, _, target_tg = payload.partition(":")
                target_tg_id = int(target_tg)
                if role == "deny":
                    await self.access.deny_access_request(target_tg_id, by_user_id=user_id)
                    await q.edit_message_text(f"🛑 Denied access for `{target_tg_id}`.",
                                              parse_mode=ParseMode.MARKDOWN)
                else:
                    await self.access.grant_role(target_tg_id, role.upper(), by_user_id=user_id)
                    await q.edit_message_text(
                        f"✅ Granted *{role.upper()}* to `{target_tg_id}`.",
                        parse_mode=ParseMode.MARKDOWN,
                    )
                    # ping the granted user
                    try:
                        await self._app.bot.send_message(
                            chat_id=target_tg_id,
                            text=(
                                f"✅ Access approved — role: *{role.upper()}*\n"
                                f"Send /help to see what you can do."
                            ),
                            parse_mode=ParseMode.MARKDOWN,
                        )
                    except Exception:
